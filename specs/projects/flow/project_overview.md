...
An event is an immutable fact recorded in the execution journal. Events are never consumed destructively: unlike a command, which one worker handles, an event may be observed independently by the plan and by any number of coordinators. There is one developer-facing event abstraction: `flow.Event[T]`. Every application event emitted by a handler or published externally carries a non-empty stable key; both paths share the same execution-scoped idempotency rule.

The runtime automatically records an event carrying a successful worker result. That automatic event follows the command-result size limit; the smaller application-event limit applies only to facts explicitly emitted or published. It records facts such as `CommandFailed`, `CommandCancelled`, `CommandExpired`, or `CommandSkipped` when a command ends another way. Workers may emit additional application events, and external integrations may publish events into a running execution — for example, a webhook recording a confirmed deposit or a monitor recording a bridge delivery. Facts such as `ExecutionSucceeded` and `PlanFailed` use the same event model. These names describe what happened; they do not define separate event systems or developer concepts.
...
The initial plan pass is deferred through the same dirty queue as later passes. This keeps plan CPU and defects out of caller-facing and caller-owned transactions and gives the runtime one capacity, recovery, and reconciliation path. It adds one scheduler hop per plan-dependent stage: notification hints normally keep that small, while poll-only operation may add up to one poll interval. Already-declared dependency chains and persisted `Await` conditions still become ready in the triggering transaction and do not pay a plan hop merely to release stored topology.
...
- Event-key identity is reserved across versions, while waits and subscriptions match exact versions; deployments retain the version expected by each in-flight execution until it finishes or is deliberately replaced by a new execution.
