...
Commands and events belong to an **execution**. A plan is optional. The simplest execution starts one root command directly; its workers may form a bounded command tree with `Spawn`, and the execution finishes when that tree settles. When progression requires dependencies, joins, waits, or branches across commands, a **plan** declares that orchestration as a pure function evaluated over all relevant events and command results recorded so far. Events and terminal command outcomes durably enqueue plan reconciliation; several inputs may coalesce before a compatible plan replica evaluates the complete snapshot. "React" does not mean that the plan receives one event callback. Where membership is open-ended or a plan cannot express the logic, a hand-written **coordinator** reacts to events directly.
...
This is a product feature: a declared short application commit function, command completion, emitted events, and worker-spawned children can share one transaction. A plan-triggering transition atomically marks durable reconciliation work; the resulting plan declaration batch commits in a later short transaction. PostgreSQL notifications may reduce latency, but polling is always sufficient for correctness.
...
- atomic outputs within each worker, plan-reconciliation, and coordinator decision, including writes made by declared worker commit functions;
...
- `PlanDef.Execute` creates a plan-driven execution, marks it for durable plan reconciliation, and returns without invoking the plan inline.
...
Most workers register no commit function. A worker that must make a short PostgreSQL application write inseparable from its successful command outcome may register one declared function with `WithCommit` (§4.2). The runtime invokes it only after the handler returns `(result, nil)`, output and fence validation succeed, and the short settlement transaction has begun. Its `Commit[A, R]` value contains canonical durable arguments, the successful typed result, and command metadata. The application write, result event, journal entries, staged outputs, materialized command state, and plan-dirty marker then commit or roll back together. Any commands the plan declares in response commit later in the plan reconciler's own atomic transaction.
...
Starting the plan marks the new execution dirty and returns. A plan-capable runtime claims that durable reconciliation work and the first evaluation declares only `prepare`, whose creation is appended to the execution journal with its arguments, dependencies, origin, and causation. Its worker discovers the complete analysis membership and stages every child with `Spawn`. The declared commit function then marks the application report prepared using only the durable command arguments and result. That write, every child-creation journal entry and command row, the event returned by `PrepareReport.Done()`, closed membership, and the dirty marker for another plan pass become visible atomically. The next reconciler pass sees `Children` and declares every failure branch and the pending `generate` command together. Analysis workers run in parallel. Once every `After` dependency succeeds, `generate` becomes runnable and its worker reads the immutable typed dependency results through `ResultOf`.
...
The monitor needs only the event definition and client; it does not register or execute `IntentExecution`. Publishing the fact satisfies the stored wait immediately and marks the plan-driven execution dirty. If the plan also consults that event through `Fact` or `Facts`, the same later reconciler pass sees the retained fact. Publication never depends on which inputs a previous plan evaluation happened to consult.
...
For a plan-driven execution, **successful** completion is derived: every command is terminal, no plan-declared command is pending or awaiting an unarrived fact, `plan_dirty` is false, the latest completed evaluation has no temporarily unavailable read (§10.1), and that evaluation reached a no-new-command pass. Terminal failure follows §6.3 and is not blocked by temporarily unavailable reads after its explicit failure-handling work has resolved, but it still waits for a dirty plan to reconcile so an `AfterFailed` branch cannot be missed. Because spawned children increment the same open-command count and every plan trigger durably marks reconciliation work, neither unfinished fan-out nor temporary quiescence can report false success or premature failure.
...
Deadline outcome depends on PostgreSQL acceptance time under the execution lock, not on when a maintenance sweep happens. A matching fact whose recorded acceptance time is no later than the persisted `Within` deadline satisfies the wait even if expiry maintenance runs afterward. A fact first accepted after that deadline remains immutable execution history but does not satisfy or resurrect the wait; generic maintenance records the command's expiry and marks a plan-driven execution dirty. Neither maintenance nor the publisher needs plan code.
...
A worker claims only command `(name, version)` pairs it has registered, using row-level locking that skips rows another process is claiming. It does not need the execution's plan definition: settling a plan-driven command durably records its outcome and marks the execution dirty, while a separately deployed compatible plan replica reconciles later. A process lacking the required command worker leaves the command pending, consumes no retry budget, and never fails it, so old and new versions of a service may share one database.

Unclaimable backlog—a missing command worker, dirty execution with no exact plan replica, missing coordinator definition, or matching coordinator handler—is surfaced through inspection and observability rather than stalling silently.
...
The terminal plan-defect rule applies to the first and every later reconciliation. `PlanDef.Execute` and ingress return after durably marking plan work and do not synchronously surface a later defect. The library-owned reconciler preserves the already accepted execution, command outcomes, and facts for inspection, records the typed plan failure against that `ExecutionID`, and makes it visible through the handle, `AwaitExecution`, `Trace`, and observations.

`InTx` never evaluates a plan and therefore never discovers a plan defect. It may atomically append an event or terminal command transition and mark the execution dirty inside the caller-owned transaction. If the caller rolls back, both the trigger and dirty marker disappear; if it commits, a later library-owned reconciliation records any resulting plan defect without undoing the caller's application write.
...
Every command made runnable by one evaluation is created in that single reconciliation transaction, so a crash exposes either all of them or none.

Reconciliation continues to a bounded fixed point only when applying a declaration batch itself creates a new plan-observable terminal transition, such as a command that is immediately skipped or expired. The engine re-evaluates against that in-transaction outcome and repeats until no such transition opens another branch. Those in-batch terminal outcomes are consumed by the fixed point and do not leave the execution dirty again after the final clear. Creating ordinary `ready` or `pending` work does not force a redundant second pass: that open work already prevents completion, and its later terminal transition supplies the next required evaluation. One durable transition may therefore invoke the pure plan more than once, but invocation count is never application-visible behavior.
...
After a successful fixed point, the same transaction increments `plan_revision`, stores whether the plan reached a no-new-command pass, stores the exact count of temporarily unavailable reads, records at most 32 safe inspection-only summaries of what it is waiting for, and clears `plan_dirty`. The complete consulted-input set is not persisted and does not participate in routing. If evaluation panics or produces a deterministic defect, that library-owned transaction records `PlanFailed`, fails the execution, and clears the dirty work rather than retrying an invalid decision forever.

### 10.3 When the plan is evaluated

A plan is queued for reconciliation:

- when its execution is created;
- when any application event is appended in that execution;
- when any command reaches a terminal state, including success, failure, cancellation, expiry, or skipping; and
- when a reconciliation-created immediate terminal transition requires another in-transaction fixed-point pass.

The first three triggers only set an already-true dirty bit, so bursts naturally coalesce and never create one queue row per event. Plan-capable runtimes probe dirty executions by exact definition pair, reserve bounded local plan capacity, and use `FOR UPDATE SKIP LOCKED`; different executions reconcile concurrently while one execution remains serialized by its ordinary row lock. Transactional notifications are hints, and polling alone recovers every dirty execution.

The plan is pure and reads the complete durable snapshot, so coalescing cannot lose an intermediate fact: events and terminal outcomes are immutable and remain re-readable. Reconciliation may run more often than logically necessary, but repeated equivalent `Do` declarations coalesce by key and append no duplicate history. No persisted reactive subscription set or per-read routing table is maintained.

If no live runtime registers the exact plan version, the execution remains durably dirty, existing command workers may continue to run and settle, and inspection reports `missing_plan_definition`. Once a compatible plan replica appears, it reconciles all accumulated inputs. Derived success or failure cannot complete while dirty; explicit execution cancellation and the execution deadline may instead clear dirty work while terminalizing the execution.
...
Immediate terminal cascades and lazy durable-selector/value loading may invoke a plan more than once in one semantic transaction, but every additional fixed-point pass must follow at least one genuinely new terminal declaration. This does not change the approximately O(commands²) lifetime bound when the command ceiling is enabled. A high implementation safety guard bounds total plan invocations in one transaction, including loading and fixed-point passes; it is a transaction-resource guard, not a queue, concurrency, or total-command limit, and a plan that exceeds it fails deterministically rather than holding the execution lock indefinitely.
...
That one transaction appends the event carrying the command result, its additionally emitted events, a `CommandCreated` entry for every staged child, attempt conclusion, and any engine-derived transition; executes the worker's optional declared commit function; materializes the complete child set, dependencies, command state, and execution state; and marks a plan-driven execution dirty when the new facts can affect its next snapshot. If ordinary settlement fails, none commits and the command is retried.

Plan reconciliation is a separate short library-owned transaction. It locks the dirty execution, runs the pure plan over the complete committed snapshot, inserts and journals the whole declaration batch, resolves immediate topology, and clears the dirty marker only after reaching its bounded fixed point. A plan panic, nondeterministic conflict, invalid read, or deterministic limit violation records `PlanFailed` and `ExecutionFailed` in that transaction and cancels outstanding work. It never rolls back or reruns an already committed worker result or commit-function write.
...
Every non-terminal command has a bounded path to terminal: direct eligibility, dependencies that will resolve, an awaited fact with a deadline, a retry schedule, a `Within` bound, or the execution deadline. Direct executions additionally have bounded topology because every worker publishes its complete direct-child set and closes membership in one successful return and the execution-wide command ceiling bounds recursive generations unless the operator explicitly disables it. A background reconciler repairs derived queue state after crashes without duplicating work.

Three situations lie outside this and are reported rather than absorbed: an execution started with `WithoutExecutionDeadline` whose commands declare no bounds; an execution whose command ceiling is explicitly disabled and whose handlers or coordinator continue creating work indefinitely; and work for which no live replica registers the required capability—its command worker, the exact plan needed by a dirty execution, or its coordinator definition. Unknown work remains durable and consumes no retry budget while that deployment gap exists.
...
Outputs created inside handlers automatically inherit execution identity and causation: worker events and spawned direct children are caused by the current command; plan outputs are caused by the `PlanReconciled` decision record, which identifies the complete journal prefix it consumed; coordinator outputs are caused by the start activation or event being processed; an event recording a command's final state identifies that command and transition. Callers cannot forge an origin that contradicts the active handler scope.
...
Command-worker pools, plan-reconciliation pools, API processes, and external monitors may be deployed independently. A command worker never needs the execution's plan code: its terminal event atomically marks the execution dirty. Only a runtime consuming dirty plan executions registers the exact plan version. API processes and publishers need no plan registration. Coordinator-driven command settlement is similarly independent of coordinator code because its terminal event may wait durably for a compatible coordinator replica.
...
| plan-dirty time | when was the first not-yet-reconciled plan trigger accepted? | clean-to-dirty transition only; cleared after reconciliation |
...
- a plan-queue harness coalesces several synthetic triggers into one dirty execution, reconciles the complete resulting snapshot, and verifies that equivalent repeated evaluation creates no duplicate work;
...
Integration behavior is verified against real PostgreSQL: concurrent command and dirty-plan claims, lease expiry and fencing, cancellation races, crash recovery at every commit boundary, immutable retry-budget anchors across retry, interruption, restart, and takeover, elapsed-budget exhaustion independent of attempt count, changed definition defaults leaving existing commands untouched, explicit node-policy conflict detection, dependency-gated `Within` anchors, PostgreSQL-anchored `StartAfter` scheduling and deadline expiry, publish-before-declare and declare-before-publish ordering, atomic external-monitor fact publication with its application write and dirty marker, coalesced plan triggers, plan recovery after a reconciler crash, independent deployment of publishers, workers, and plan replicas, Flow-before-application and ascending multi-execution transaction ordering, exactly one command-creation journal entry per accepted command, execution-wide command-ceiling enforcement for root, `Do`, `Spawn`, and `Issue`, journal/graph reconstruction, all-or-nothing worker, plan-batch, and coordinator outputs, authoritative child reads, batched worker dependency results and terminal outcomes, immediate-terminal fixed-point reconciliation, repeated plan evaluation creating no duplicate commands or history, unknown dependency rejection, terminally unavailable results not blocking failure, failure branches surviving fail-fast, typed coordinator fan-in across successful and unsuccessful command outcomes, durable-agent continuation after process loss, `Await` expiry, and rolling deployments with divergent registered versions.
...
- every application fact in a plan-driven execution commits independently of plan availability, resolves persisted `Await` rows immediately, and atomically sets `plan_dirty`; publishers never need to register the plan;
...
- a plan-driven execution succeeds exactly when every declared command is terminal, nothing remains pending or awaited, `plan_dirty` is false, no read in the latest completed evaluation is temporarily unavailable, and that evaluation declared nothing new; it never succeeds on temporary quiescence;
...
- a plan panic, conflicting declaration, or invalid read records `PlanFailed` in the plan reconciler transaction and fails the execution without consuming worker retry budget or undoing previously committed worker/commit-function work;
...
- plan evaluation at the documented command ceiling stays within its benchmarked budget; many events and outcomes may coalesce behind one dirty bit, and the subsequent evaluation sees every retained fact and terminal outcome;
- a command result, emitted events, spawned-child journal entries and materializations, dependency resolution, execution transitions, declared commit-function writes, and the plan-dirty marker commit atomically or not at all; plan-created commands commit atomically as a separate reconciliation batch;
...
- `InTx` never runs plan code: its trigger and dirty marker commit or roll back together, and any later plan defect is recorded only by the library-owned reconciler without undoing the caller transaction;
