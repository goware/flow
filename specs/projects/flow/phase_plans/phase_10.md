...
2. In `deliver_test.go`, exercise delivery from an active producer worker into a separate exact-event-gated execution, including `ReadEvent`, source retry, idempotent re-delivery, independent commit, and preservation of the `Event.Emit` guard.
...
- `TestDeliverFromActiveWorker`: an active producer worker delivers to a separate gated execution, the target reads the payload, a failed first source attempt does not retract it, retry delivery is idempotent, and ordinary `Event.Emit` remains rejected in attempt context.
...
- `TestDeliverMultiProducerFanIn`: three independent producer executions deliver exact events that satisfy one command-owned AND join, whose worker reads every payload.
