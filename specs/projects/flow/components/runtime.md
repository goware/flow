...
`Run` starts one bounded command scheduler, command lease renewal, wait-expiry/deadline/recovery maintenance, optional reconnecting notification listening, and graceful shutdown.
...
Active attempts retain IDs, tokens, and cancellation functions. Renewers extend leases in bounded batches; fence loss cancels local context. Maintenance recovers expired leases so another replica can retry safely.
...
Observers receive bounded command, execution, event-ingress, maintenance, notification, and shutdown facts. Fault hooks cover journal, projection, queue, fence, commit, notification, and ambiguous-commit boundaries.
