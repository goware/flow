...
Observers receive bounded command, run, event-ingress, renewal classification/local cancellation, maintenance probe/transition, notification, and shutdown facts, plus the terminal run, command, wait-expiry, and lease-recovery facts an alert consumer pages on. Functional specification section 14.1 enumerates every legal `(Kind, Operation, Outcome)` tuple, its delivery class, and the additive-only compatibility policy. Terminal-class facts hold reserved queue capacity so a duty-cycle flood cannot evict them, and the drain reports dropped terminal facts separately. Delivery and shutdown drain remain best-effort and secret-free. Observers must return promptly and should honor context cancellation; a blocked callback cannot block runtime shutdown. Fault hooks cover journal, projection, queue, fence, commit, notification, and ambiguous-commit boundaries.
